...
def scrape():
	...
	# Open positions
	WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.XPATH, '//*[@id="positions-block"]/a[1]')))
	search_body = bs(driver.page_source, 'lxml')
	job_table = search_body.find('div', {'class':'positions-block'})
	jobs = job_table.find_all('div', {'class':'single-position'})
	...
	if len(jobs) != 0:
		for job in jobs:
			...
			team = job.find('div', {'class':'team'}).text.split('\n')[3].strip()
