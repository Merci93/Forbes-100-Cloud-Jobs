...
def scrape():
	...
	WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CLASS_NAME, 'eWYkWf')))

	search_body = bs(driver.page_source, 'lxml')
	job_table = search_body.find_all('div', {'class':'eKiWiO'})
	jobs = job_table[2].find_all('div', {'class':'lnSrdb'})
	...
	if len(jobs) != 0:
		for job in jobs:
			try:
				...
				location = [item.text for item in job.find_all('a')]
